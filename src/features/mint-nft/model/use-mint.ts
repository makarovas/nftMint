...
export function useMint() {
  ...
  const [method, setMethod] = useState<'native' | 'erc20' | 'igCoin'>('native');
  ...
  const { data: igCoinBalance } = useBalance({
    address,
    token: CONTRACTS.IG_COIN.address as `0x${string}`,
  });
  ...
  const { write: mintWithIGCoin, data: mintIGCoinData } = useContractWrite({
    address: CONTRACTS.NFT.address as `0x${string}`,
    abi: NFT_ABI,
    functionName: 'mintWithIGCoin',
  });
